...
# Tambahkan direktori utama (root) ke path agar bisa mengimpor dari core
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

# Impor fungsi dari lokasi modular yang benar
...
def test_is_valid_domain_failure():
    ...
    assert not is_valid_domain("http://google.com") # Seharusnya tidak mengandung protokol
    assert not is_valid_domain("google.com/") # Seharusnya tidak mengandung path
    ...
    assert not is_valid_domain("domain.123") # TLD tidak boleh hanya angka
